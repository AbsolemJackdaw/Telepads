...
public class GuiNameTelepad extends GuiScreen {
	...
	public void initGui() {
		...
		String padName = te.getTelePadName().equals("TelePad") ? te
				.getWorldObj().getBiomeGenForCoords(te.xCoord, te.zCoord).biomeName
				: te.getTelePadName();
	...
	public void sendPacket(String padName) {
		...
		Telepads.instance.network.sendToServer(new PacketAddTelepadEntry(
				mc.thePlayer.getUniqueID(), new TelepadEntry(padNameField
						.getText(), mc.thePlayer.worldObj.provider.dimensionId,
						new Position(te.xCoord, te.yCoord, te.zCoord), false,
						false)));
