...
public class GuiTeleport extends GuiScreen {
	...
	/**
	 * int : buttonID to keep track of entry to look for in player locations,
	 * TelepadEntry
	 */
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		List<TelepadEntry> telepads = PlayerLocations.getProperties(player)
				.getEntries();
		...
		if (!te.hasDimensionUpgrade()) {
			fontRendererObj.drawSplitString(
					te.getWorldObj().provider.getDimensionName(), 48 + 1,
					11 + 1, 180, 0x000000);
			fontRendererObj.drawSplitString(
					te.getWorldObj().provider.getDimensionName(), 48, 11, 180,
					0xffffff);
		} else {
			WorldProvider world = null;

			try {
				world = te.getWorldObj().provider
						.getProviderForDimension(dimension_ID);
			} catch (Exception e) {
			}

			if (world != null) {
				fontRendererObj.drawSplitString(world.getDimensionName(),
						48 + 1, 11 + 1, 180, 0x000000);
				fontRendererObj.drawSplitString(world.getDimensionName(), 48,
						11, 180, 0xffffff);
			} else {
				fontRendererObj.drawSplitString("No%Dim- Error : Hz "
						+ dimension_ID, 48 + 1, 11 + 1, 180, 0x000000);
				fontRendererObj.drawSplitString("No%Dim- Error : Hz "
						+ dimension_ID, 48, 11, 180, 0xffffff);
			}
		}
	}
	...
	public void sendPacket(int id) {
		...
		int x = PlayerLocations.getProperties(player).getEntries().get(id).position
				.getX();
		int y = PlayerLocations.getProperties(player).getEntries().get(id).position
				.getY();
		int z = PlayerLocations.getProperties(player).getEntries().get(id).position
				.getZ();
		int dim = PlayerLocations.getProperties(player).getEntries().get(id).dimensionID;

		Telepads.instance.network.sendToServer(new PacketTeleport(new Position(
				x, y, z), dim, new Position(te.xCoord, te.yCoord, te.zCoord),
				false));
	}
	...
	private void makePage() {
		...
		for (TelepadEntry tpe : pageEntries.keySet()) {
			...
			String name = flag ? ChatFormatting.RED + " " + tpe.entryName
					: flag2 ? ChatFormatting.GREEN + " " + tpe.entryName
							: tpe.entryName;

			this.buttonList
					.add(new GuiButton(
							pageEntries.get(tpe), /* x */
							(40) + (((i / 10) > 0) && ((i % 10) >= 0) ? 120 * (i / 10)
									: 0), /* y */
							(130 + ((i * 25)))
									- (((i / 10) > 0) && ((i % 10) >= 0) ? (250 * (i / 10)) + 100
											: 100), /* size */100, 20, name));
			i++;
		}
	}
}
