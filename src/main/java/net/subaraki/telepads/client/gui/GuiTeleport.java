...
public class GuiTeleport extends GuiScreen {
    ...
    public void sendPacket (int id) {
        ...
        Telepads.instance.network.sendToServer(new PacketTeleport(new Position(x, y, z), dim, new Position(te.xCoord, te.yCoord, te.zCoord), false));
    }
    ...
    private void makePage () {
        ...
        for (TelepadEntry tpe : pageEntries.keySet()) {
            ...
            this.buttonList.add(new GuiButton(pageEntries.get(tpe), /* x */
                    (40) + (((i / 10) > 0) && ((i % 10) >= 0) ? 120 * (i / 10) : 0), /* y */
                    (130 + ((i * 25))) - (((i / 10) > 0) && ((i % 10) >= 0) ? (250 * (i / 10)) + 100 : 100), /* size */100, 20, name));
            i++;
        }
    }
}
