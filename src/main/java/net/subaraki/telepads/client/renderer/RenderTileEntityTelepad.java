...
public class RenderTileEntityTelepad extends TileEntitySpecialRenderer {
	...
	public void renderEndPortalSurface (double par2, double par4, double par6, float par8) {
		...
		for (int i = 0; i < 16; ++i) {
			GL11.glPushMatrix();
			float f5 = 16 - i;
			float f6 = 0.0625F * 5f; // looking inside makes the stars go smaller, and more numerous
			float f7 = 1.0F / ((f5 / 10) + 1.0F);// Colours the stars less or more bright
			...
			GL11.glTexGen(GL11.GL_S, GL11.GL_OBJECT_PLANE, this.createBuffer(1.0F, 0.0F, 0.0F, 0.0F));
			GL11.glTexGen(GL11.GL_T, GL11.GL_OBJECT_PLANE, this.createBuffer(0.0F, 0.0F, 1.0F, 0.0F));
			GL11.glTexGen(GL11.GL_R, GL11.GL_OBJECT_PLANE, this.createBuffer(0.0F, 0.0F, 0.0F, 1.0F));
			GL11.glTexGen(GL11.GL_Q, GL11.GL_EYE_PLANE, this.createBuffer(0.0F, 1.0F, 0.0F, 0.0F));
	...
	private void renderTorch (TileEntityTelepad te, double offsetX, double offsetY, double offsetZ, double x, double y, double z) {
		...
		tessellator.startDrawingQuads();
		tessellator.setTranslation(x - te.xCoord - offsetX, y - te.yCoord - offsetY, z - te.zCoord - offsetZ);
	...
	/**
	 * Renders a telepad model. this is rendered without norm and without end-portal sky texture
	 * */
