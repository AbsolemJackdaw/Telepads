...
public class TeleportUtility {
	...
	private static Entity teleportEntity(Entity entity, double x, double y,
			double z, int dimDst, boolean forceRecreate) {
		if (entity == null || entity.isDead == true
				|| entity.worldObj.isRemote == true) {
			return null;
		}

		// Post the event and check if the teleport should be allowed
		if (entity instanceof EntityLivingBase) {
			EnderTeleportEvent etpEvent = new EnderTeleportEvent(
					(EntityLivingBase) entity, x, y, z, 0.0f);
		...
		if (entity.worldObj.isRemote == false
			...
			WorldServer worldServerDst = minecraftserver
			...
			if (chunkProvider.chunkExists((int) x >> 4, (int) z >> 4) == false) {
				...
				if (entity.dimension != dimDst
						|| (entity.worldObj instanceof WorldServer && entity.worldObj != worldServerDst))
					entity = TeleportUtility.transferEntityToDimension(entity,
							dimDst, x, y, z);
				else {
					...
					// one of them
						...
						entity = TeleportUtility
								.reCreateEntity(entity, x, y, z);
					else if (entity instanceof EntityLivingBase)
						((EntityLivingBase) entity).setPositionAndUpdate(x, y,
								z);
					else
	...
	public static Entity reCreateEntity(Entity entitySrc, double x, double y,
			double z) {
		if (entitySrc.worldObj.isRemote == true)
			return null;

		WorldServer worldServerDst = MinecraftServer.getServer()
				.worldServerForDimension(entitySrc.dimension);
		if (worldServerDst == null) {
			Telepads.printDebugMessage("reCreateEntity(): worldServerDst == null");
			return null;
		}

		Entity entityDst = EntityList.createEntityByName(
				EntityList.getEntityString(entitySrc), worldServerDst);
		if (entityDst == null)
			return null;
		...
		if (entityDst instanceof EntityLivingBase)
			...
			entityDst.setLocationAndAngles(x, y, z, entitySrc.rotationYaw,
					entitySrc.rotationPitch);
	...
	public static Entity transferEntityToDimension(Entity entitySrc,
			int dimDst, double x, double y, double z) {
		if (entitySrc == null || entitySrc.isDead == true
				|| entitySrc.dimension == dimDst
				|| entitySrc.worldObj.isRemote == true)
			return null;

		if (entitySrc instanceof EntityPlayerMP)
			return TeleportUtility.transferPlayerToDimension(
					(EntityPlayerMP) entitySrc, dimDst, x, y, z);

		WorldServer worldServerSrc = MinecraftServer.getServer()
				.worldServerForDimension(entitySrc.dimension);
		WorldServer worldServerDst = MinecraftServer.getServer()
				.worldServerForDimension(dimDst);
	...
	public static EntityPlayerMP transferPlayerToDimension(
			EntityPlayerMP player, int dimDst, Position pos) {
		return transferPlayerToDimension(player, dimDst, pos.getX(),
				pos.getY(), pos.getZ());
	}

	public static EntityPlayerMP transferPlayerToDimension(
			EntityPlayerMP player, int dimDst, double x, double y, double z) {
		...
		// Post the event and check if the teleport should be allowed
		PlayerChangedDimensionEvent pcdEvent = new PlayerChangedDimensionEvent(
				player, player.dimension, dimDst);
		...
		ServerConfigurationManager serverCM = player.mcServer
				.getConfigurationManager();
		WorldServer worldServerSrc = MinecraftServer.getServer()
				.worldServerForDimension(dimSrc);
		WorldServer worldServerDst = MinecraftServer.getServer()
				.worldServerForDimension(dimDst);
		...
		player.mcServer.getConfigurationManager()
				.updateTimeAndWeatherForPlayer(player, worldServerDst);
		...
		while (iterator.hasNext()) {
			...
			player.playerNetServerHandler.sendPacket(new S1DPacketEntityEffect(
					player.getEntityId(), potioneffect));
		}
