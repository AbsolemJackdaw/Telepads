...
public class PlayerLocations implements IExtendedEntityProperties {
	...
	/**
	 * The name of the NBTTagCompound which holds all of the data handled by
	 * this class.
	 */
	...
	/**
	 * Constructs a new PlayerLocations instance. This should only be used when
	 * creating new properties for a player. To add properties to a player, look
	 * at setProperties.
	 * 
	 * @param player
	 *            : The player that the new PlayerLocations instance is for.
	 */
	...
	public void loadNBTData(NBTTagCompound compound) {
		...
		for (int tagPos = 0; tagPos < entryTagList.tagCount(); tagPos++)
			entryList.add(new TelepadEntry(entryTagList
					.getCompoundTagAt(tagPos)));
	 ...
	 * 
	 * @param player
	 *            : An instance of the player to retrieve the properties from.
	 * @return PlayerLocations: The found instance of PlayerLocations. This will
	 *         be null if none is found.
	 */
	 ...
	 * 
	 * @param player
	 *            : An instance of the player to set properties to.
	 * @return PlayerLocations: An instance of the PlayerLocations set to the
	 *         player.
	 */
	public static PlayerLocations setProperties(EntityPlayer player) {

		player.registerExtendedProperties(PROP_NAME,
				new PlayerLocations(player));
	 ...
	 * 
	 * @param player
	 *            : The instance of the player to check.
	 * @return boolean: If the player has a valid instance true will be
	...
	/**
	 * Sets all of the stored properties, to reflect those of another instance
	 * of PlayerLocations.
	 * 
	 * @param properties
	 *            : The instance of PlayerLocations to pull everything from.
	 */
	 ...
	 * 
	 * @return List<TelepadEntry>: A list of every TelepadEntry stored on the
	 *         players file.
	 */
	...
	/**
	 * Sets the list of all TelepadEntrys to a new list of entries. This will
	 * delete all previous entries. This will also sync all server side data to
	 * the client.
	 * 
	 * @param entries
	 *            : The new list of entries that will replace the old ones.
	 */
	...
	/**
	 * Adds a new TelepadEntry to the players list of entries. This will also
	 * sync all server side data to the client.
	 * 
	 * @param entry
	 *            : The new TelepadEntry to add for the player.
	 */
	...
	/**
	 * Removes a specified TelepadEntry from the players list of entries. This
	 * will also sync all server side data to the client.
	 * 
	 * @param entry
	...
	/**
	 * Synchronizes the data between the server and the client. In some cases
	 * the client's version of the PlayerLocation may be out of sync. Calling
	 * this will fix that.
	 */
	public void sync() {

		if (player instanceof EntityPlayerMP)
			Telepads.instance.network.sendTo(new PacketSyncTelepadEntries(
					player.getUniqueID(), this.entries),
					(EntityPlayerMP) player);
	}

	/**
	 * Provides a suggested TelepadEntry name based on the players dimension,
	 * and existing entries.
	 * 
	 * @return String: A name that can be used for a TelepadEntry. Example:
	 *         Surface 9
	 */
	...
	public static class TelepadEntry {
		...
		/**
		 * Creates a TelepadEntry from a ByteBuf. This is useful for reading
		 * from networking.
		 * 
		 * @param buf
		 *            : A ByteBuf containing the data needed to create a
		 *            TelepadEntry.
		 */
		public TelepadEntry(ByteBuf buf) {

			this(ByteBufUtils.readUTF8String(buf), buf.readInt(), new Position(
					buf), buf.readBoolean(), buf.readBoolean());
		}

		/**
		 * Creates a TelepadEntry from a NBTTagCompound. This is used for
		 * reading from NBTTagCompound.
		 * 
		 * @param tag
		...
		public TelepadEntry(NBTTagCompound tag) {
			...
			this(tag.getString("entryName"), tag.getInteger("dimensionID"),
					new Position(tag), tag.getBoolean("power"), tag
							.getBoolean("transmitter"));
		}

		/**
		 * Creates a new TelepadEntry. This is used to represent an entry that a
		 ...
		 * @param pos
		 *            : The Position of this TelepadEntry.
		 * @param isPowered
		 *            defaults to false. wether this entry's tile entity is
		 *            redstone powered or not
		 * @param hasTransmitter
		 *            defaults to false. wether this entry's tile entity has a
		 *            transmitter upgrade
		 */
		public TelepadEntry(String name, int dimension, Position pos,
				boolean isPowered, boolean hasTransmitter) {
		 ...
		 * @return NBTTagCompound: An NBTTagCompound containing all of the
		 *         TelepadEntry data.
		 */
		...
		public String toString() {
			...
			return "Entry Name: " + this.entryName + " DimensionID: "
					+ this.dimensionID + " " + this.position.toString();
		}

		@Override
		public Object clone() {

			return new TelepadEntry(this.entryName, this.dimensionID,
					this.position, this.isPowered, this.hasTransmitter);
		}
		...
		public boolean equals(Object compared) {
			...
			return this.entryName.equals(entry.entryName)
					&& this.dimensionID == entry.dimensionID
					&& this.position.equals(entry.position);
		}
