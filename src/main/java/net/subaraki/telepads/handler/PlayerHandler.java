...
public class PlayerHandler {
	...
	public void onPlayerClone(PlayerEvent.Clone event) {
		...
		PlayerLocations.getProperties(event.entityPlayer).copy(
				PlayerLocations.getProperties(event.original));
	}
	...
	public void onEntityConstructing(EntityConstructing event) {
		...
		if (event.entity instanceof EntityPlayer
				&& !PlayerLocations.hasProperties((EntityPlayer) event.entity))
		...
		if (event.current != null)
			if (event.current.getItem().equals(Items.water_bucket)) {
				if (event.target.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
					Block b = event.world.getBlock(event.target.blockX,
					...
					if (b instanceof BlockTelepad) {
						TileEntityTelepad telepad = (TileEntityTelepad) event.world
								.getTileEntity(event.target.blockX,
										event.target.blockY,
										event.target.blockZ);

						if (event.entityPlayer.isSneaking())
							telepad.setBaseColor(new Color(243, 89, 233)
									.getRGB());
						if (!event.entityPlayer.isSneaking())
							telepad.setFrameColor(new Color(26, 246, 172)
									.getRGB());
	...
	public void onPlayerInteraction(PlayerInteractEvent event) {
		...
		if (event.action.equals(Action.RIGHT_CLICK_BLOCK)
				&& event.world.getBlock(event.x, event.y, event.z) instanceof BlockTelepad) {
			...
			if (event.entityPlayer.getHeldItem() != null) {
				...
				if (event.entityPlayer.getHeldItem().getItem()
						.equals(Telepads.transmitter)
						&& !telepad.hasDimensionUpgrade()) {

					PlayerLocations playerLocations = PlayerLocations
							.getProperties(event.entityPlayer);

					for (TelepadEntry tpe : playerLocations.getEntries())
						if (tpe.position.equals(new Position(event.x, event.y,
								event.z)))
					...
					if (!event.entityPlayer.capabilities.isCreativeMode)
						event.entityPlayer.getHeldItem().stackSize--;
					telepad.markDirty();
				} else if (event.entityPlayer.getHeldItem().getItem()
