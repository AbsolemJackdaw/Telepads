...
public class PacketTeleport implements IMessage {
     ...
     * @param forceTeleport : Flag to force teleport and bypass checking if a tile entity
    ...
    public static class PacketTeleportHandler implements IMessageHandler<PacketTeleport, IMessage> {
        ...
        public IMessage onMessage (PacketTeleport packet, MessageContext ctx) {
            ...
            if (packet.dimension == player.dimension) {
                ...
                /* this code will return null if dimension is unloaded */
                /* WorldServer worldToCheck = DimensionManager.getWorld(packet.dimension); */
                
                /* masa provided me with this snip of code that works */
                WorldServer worldToCheck = MinecraftServer.getServer().worldServerForDimension(packet.dimension);
                ...
                if (worldToCheck != null) {
                    ...
                    player.addChatMessage(new ChatComponentText("Could not teleport to other dimension " + packet.dimension));
            }
            return null;
        }
    }
