...
public class PacketSyncTelepadEntries implements IMessage {
	...
	/**
	 * A packet to sync entries to a player, on the client side. This packet
	 * must only be sent from a server thread. The entries on the client side
	 * will be overridden with the provided list of entries.
	 * 
	 * @param playerUUID
	 *            : The unique identifier of the player sync this data to.
	 * @param entries
	...
	public static class PacketSyncTelepadEntriesHandler implements
			IMessageHandler<PacketSyncTelepadEntries, IMessage> {

		@Override
		public IMessage onMessage(PacketSyncTelepadEntries packet,
				MessageContext ctx) {

			EntityPlayer player = Utilities.getPlayerFromUUID(
					Utilities.thePlayer().worldObj, packet.playerUUID);
			PlayerLocations.getProperties(player).overrideEntries(
					packet.entries);
			return null;
		}
	}
