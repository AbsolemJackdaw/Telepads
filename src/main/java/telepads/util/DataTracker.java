...
public class DataTracker {
	...
	private void syncClient(EntityPlayer p) {
		...
		for (int i = 0; i < PlayerPadData.get(p).getAllCoords().size(); i++) {
			...
			if (!p.worldObj.isRemote)
				Telepads.SNW.sendTo(new PacketDataOverDimensions(PlayerPadData
						.get(p).getAllCoords().get(i)[0], PlayerPadData.get(p)
						.getAllCoords().get(i)[1], PlayerPadData.get(p)
						.getAllCoords().get(i)[2], PlayerPadData.get(p)
						.getAllDims().get(i), PlayerPadData.get(p)
						.getAllNames().get(i)), (EntityPlayerMP) p);
		}
	}
}
