...
public class BlockTelepad extends BlockContainer {
	...
	protected void dropBlockAsItem(World p_149642_1_, int p_149642_2_,
		...
		super.dropBlockAsItem(p_149642_1_, p_149642_2_, p_149642_3_,
				p_149642_4_, p_149642_5_);
	}
	...
	public boolean onBlockActivated(World par1World, int x, int y, int z,
		...
		/**
		 * ===================If the player clicking the pad IS the
		 * owner=================
		 */
				...
				if (!te.lockedUniversal) {
					if (par5EntityPlayer.getCurrentEquippedItem() != null)
						if (par5EntityPlayer.getCurrentEquippedItem().getItem() == Telepads.crowbar) {
							...
							EntityItem ei = new EntityItem(par1World, x, y, z,
									stack);
					...
					par5EntityPlayer
							.addChatComponentMessage(new ChatComponentText(
									"This Universal Pad got locked and"
			...
			// sneaking
				...
				if (!te.isUniversal) {
					if (!par1World.isRemote) {
						par5EntityPlayer
								.addChatComponentMessage(new ChatComponentText(
										"Your portal got opened to Universal Acces"));
						par5EntityPlayer
								.addChatComponentMessage(new ChatComponentText(
										"Your Friends can now add your pad to their coordinates."));
						par5EntityPlayer
								.addChatComponentMessage(new ChatComponentText(
										"Pad will be locked to Universal if other players register"));
					}
					te.isUniversal = true;
					par1World.markBlockForUpdate(te.xCoord, te.yCoord,
							te.zCoord);
		...
		/**
		 * ===================If the player clicking the pad is NOT the
		 * owner=================
		 */
		else {
			...
			if (par5EntityPlayer.getCurrentEquippedItem() != null) {
				...
				for (int[] coords : data.getAllCoords()) {
					if ((coords[0] == a[0]) && (coords[1] == a[1])
							&& (coords[2] == a[2])) {
						if (!par1World.isRemote) {
							par5EntityPlayer
									.addChatComponentMessage(new ChatComponentText(
											"I already registered this pad"));
						}
						return true;
					}
				}
				...
				if (!par1World.isRemote) {
					par5EntityPlayer
							.addChatComponentMessage(new ChatComponentText(
									"Registered Universal Pad to Register"));
				}
			}
		}
		return true;
	...
	public void onBlockPlacedBy(World par1World, int x, int y, int z,
		...
		if (par1World.provider.dimensionId == 1) {
			par1World.newExplosion((Entity) null, x + 0.5F, y + 0.5F, z + 0.5F,
					5.0F, true, true);
			par1World.setBlockToAir(x, y, z);
			par1World.removeTileEntity(x, y, z);
			if (!par1World.isRemote) // this should not crash. only player can
										// put this down
				((EntityPlayer) par5EntityLivingBase)
						.addChatMessage(new ChatComponentText(
								"The Magic in the End was too strong for the TelePad..."));
		...
		if (par5EntityLivingBase instanceof EntityPlayer) {
			...
			p.openGui(Telepads.instance, TelePadGuiHandler.NAMETELEPAD,
					par1World, x, y, z);
		}
	...
	public void randomDisplayTick(World par1World, int x, int y, int z,
		...
		if (te.isStandingOnPlatform) {
			for (int l = 0; l < 100; ++l) {
				...
				par1World.spawnParticle("portal", x + 0.5, d1, z + 0.5, d2, d3,
						d4);
			}
		} else {
			for (int l = 0; l < 5; ++l) {
